...
export default class VisualizarModificarContabilidadComponent extends Vue {
  ...
  async cargarList(){
    ...
    debugger;
    await diariogeneralService.getBusquedaVMAll(this.productoModel.strCompany_Cod,data.strPO_NO,data.desde,data.hasta)
    .then(res=>{
      debugger;
      for(var i=0;i<50;i++){
        this.valuem++; 
    ...
    if(this.selectrow!=undefined && this.selectrow!=null ){
      ...
      await setTimeout(() => {
        ...
        if(this.selectrow!=undefined && this.selectrow!=null ){
          ...
          router.push({ path: `/barmenu/FI-FINANZAS/contabilidad-general/modificar-contabilidad`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
        }
      }, 600)
    }
    else{
