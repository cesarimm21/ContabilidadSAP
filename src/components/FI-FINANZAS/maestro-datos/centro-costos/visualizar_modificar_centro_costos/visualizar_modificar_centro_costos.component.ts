...
export default class VisualizarModificarCentroCostosComponent extends Vue {
  ...
  async cargarList(){
    ...
    await centrocostosService.GetBusquedaCentroCostos(data.strPO_NO,data.desde,data.hasta)
    .then(res=>{
      debugger;
      for(var i=0;i<50;i++){
        this.valuem++; 
    ...
    if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.intIdInvStock_ID!=-1){
      ...
      await setTimeout(() => {
        ...
        if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.strAcc_Local_NO!=''){
          ...
          router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/centro-costos/modificar_centro_costos`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
        }
      }, 600)
    }
    else{
