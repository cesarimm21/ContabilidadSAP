...
@Component({
  name: 'modificar-centrocostos',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisualizarModificarCentroCostosComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-centrocostos';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
  ...
  async  EliminarItem(){
    if(this.documento.intIdCostCenter_ID!=-1&&this.documento.strCostCenter_NO!=""&&this.documento.strCostCenter_Desc!=""){
      this.planDialog=true;
    }
    else{
      this.warningMessage("Selecciona un Centro Costos")
    }    
  }
  inactivarPlan(){
    ...
    centrocostosService.inactivarCentroCosto(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new CentroCostosModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strCostCenter_NO!="" && this.documento.strCostCenter_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Centro Costos')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    centrocostosService.activarCentroCosto(this.documento)
    .then(resp=>{
      loadingInstance.close();
      ...
      if(this.documento.strCostCenter_NO==this.strCostCenter_NO){
        await setTimeout(() => {
          if(this.documento.strCostCenter_NO!=''){
            router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/centro-costos/modificar_centro_costos`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
