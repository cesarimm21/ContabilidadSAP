...
export default class VisualizarModificarBancoComponent extends Vue {
...
closeCategoriaCuenta(){
  ...
  async validarView(){
    ...
    if(this.selectrow!=undefined && this.selectrow!=null ){
      ...
      await setTimeout(() => {
        debugger;
        if(this.selectrow!=undefined && this.selectrow!=null ){
          router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/tesoreria/bancos/modificar-banco/modificar-banco`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
        }
      }, 600)
    }
    else{
      this.vifprogress=false;
      this.textosave='Seleccione algun banco. ';
    }
  }
  ...
  cargarList(){
    let loadingInstance = Loading.service({
      ...
      this.bancoModel.strBank_Curr=this.Currency_Cod;
      debugger;
      if(this.bancoModel.strCountry==""){
        this.bancoModel.strCountry='*';
      }
      if(this.bancoModel.strCompany_Cod==undefined){
        this.bancoModel.strCompany_Cod='*';
      }
      if(this.bancoModel.strBank_Curr==undefined){
        this.bancoModel.strBank_Curr='*';
      }
      if(this.bancoModel.strBank_Cod==""){
        this.bancoModel.strBank_Cod='*';
      }

      bancoService.busquedaBanco(this.bancoModel)
      .then(response=>{
        loadingInstance.close();
      ...
      .catch(e =>{      
        this.openMessageError('Error buscar banco');
        loadingInstance.close();
        this.textosave = 'No se pudo buscar el banco.';
        this.issave=false;
        this.iserror=true;
      })    
  }
