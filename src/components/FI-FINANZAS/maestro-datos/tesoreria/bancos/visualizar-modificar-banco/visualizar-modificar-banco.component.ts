...
export default class VisualizarModificarBancoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-banco';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strBank_Cod==this.strBank_Cod){
        await setTimeout(() => {
          if(this.documento.strBank_Cod!=''){
            router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/tesoreria/bancos/modificar-banco/modificar-banco`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
