...
export default class VisualizarModificarCuentaContableComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-cuenta-contable';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
    load(){
        ...
        cuentacontableontableService.GetAllCuentaContable(this.companyCod)
        .then(response=>{
        ...
        if(this.cuentacontable.strAcc_Local_NO==this.strAcc_Local_NO){
          await setTimeout(() => {
            if(this.cuentacontable.strAcc_Local_NO!=''){
              router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/contabilidad-general/modificar_cuenta_contable`, query: { vista:'modificar' ,data:JSON.stringify(this.cuentacontable) }  })
            }
          }, 600)
        }
        else{
