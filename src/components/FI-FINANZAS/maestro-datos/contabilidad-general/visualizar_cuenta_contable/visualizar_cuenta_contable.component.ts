...
export default class VisualizarCuentaContableComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-cuenta-contable';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
        ...
        if(this.cuentacontable.strAcc_Local_NO==this.strAcc_Local_NO){
          await setTimeout(() => {
            if(this.cuentacontable.strAcc_Local_NO!=''){
              router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/contabilidad-general/modificar_cuenta_contable`, query: { vista:'visualizar' ,data:JSON.stringify(this.cuentacontable) }  })
            }
          }, 600)
        }
        else{
