...
export default class VisualizarModificarElementoGastoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-elemento-gasto';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
  ...
  async EliminarItem(){
    if(this.cuentacontable.intIdAcctCont_ID!=-1 &&this.cuentacontable.strAcc_Local_NO!=""){
  ...
  async btnEliminar(){
    ...
    await cuentacontableontableService.inactivarCuentaContable(this.cuentacontable)
    .then(response=>{
      loadingInstance.close();
      if(response!=undefined){
         this.textosave='Se Inactivo correctamento.'+this.cuentacontable.strAcc_Local_NO;
         this.issave=true;
         this.iserror=false;
         this.$message({
          showClose: true,
          type: 'success',
          message: 'Se Inactivo correctamento '+this.cuentacontable.strAcc_Local_NO
        });
      }
      else{
        this.issave=false;
  ...
  async btnActivar(){
    ...
    this.cuentacontable.strModified_User=this.nameuser;
    if(this.cuentacontable.intIdAcctCont_ID!=-1&&this.cuentacontable.strAcc_Local_NO!=""){
      let loadingInstance = Loading.service({
        fullscreen: true,
        text: 'Activando...',
      ...
      await cuentacontableontableService.activarCuentaContable(this.cuentacontable)
      .then(respo=>{
        loadingInstance.close();
        this.successMessage('Se activo el Cuenta Contable '+this.cuentacontable.strAcc_Local_NO)
        this.load();
        this.issave=true;
        this.iserror=false;
        this.textosave='Se Inactivo el Cuenta Contable '+this.cuentacontable.strAcc_Local_NO;
        this.dialogInactivar=false;
      }).catch(ee=>{
        loadingInstance.close();
        this.issave=false;
        this.iserror=true;
        this.textosave='Error en Activar '+this.cuentacontable.strAcc_Local_NO;
        this.errorMessage('Error en Activar '+this.cuentacontable.strAcc_Local_NO)})
        this.dialogInactivar=false;
    }
    else{
      this.warningMessage('Debe de seleccionar una fila!!!');
        ...
        if(this.cuentacontable.strAcc_Local_NO==this.strAcc_Local_NO){
          await setTimeout(() => {
            if(this.cuentacontable.strAcc_Local_NO!=''){
              router.push({ path: `/barmenu/FI-FINANZAS/maestro-datos/contabilidad-general/elemento-gasto/modificar-elemento-gasto`, query: { vista:'modificar' ,data:JSON.stringify(this.cuentacontable) }  })
            }
          }, 600)
        }
        else{
