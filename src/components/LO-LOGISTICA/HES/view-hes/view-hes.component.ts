...
export default class ViewAndEditHesComponent extends Vue {
    nameComponent: string = 'visualizar-hes';
    ...
    async validarView(){
      if(this.hesSelect.strHES_NO!='' && this.hesSelect.intIdHESH_ID!=-1){
          ...
          await setTimeout(() => {
            debugger;
            if(this.hesSelect.strHES_NO!='' && this.hesSelect.intIdHESH_ID!=-1){
              router.push({ path: `/barmenu/LO-LOGISTICA/HES/viewandedit_hes`, query: { vista:'Visualizar' ,data:JSON.stringify(this.hesSelect) }  })
            }
          }, 600)
        }
        else{
