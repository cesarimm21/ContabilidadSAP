...
@Component({
  name: 'visualizar-componente-cuentas',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisuComponenteCuentasComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-componente-cuentas';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strComp_Cod==this.strComp_Cod){
        await setTimeout(() => {
          if(this.documento.strComp_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/componente_cuentas/modif_componente_cuentas`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
