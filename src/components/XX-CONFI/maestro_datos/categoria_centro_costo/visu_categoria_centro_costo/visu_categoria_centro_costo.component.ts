...
@Component({
  name: 'visualizar-categoria-centro-costos',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisuCategoriaCentroCostoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-categoria-centro-costos';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strCCCategory_Cod==this.strCCCategory_Cod){
        await setTimeout(() => {
          if(this.documento.strCCCategory_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/categoria_centro_costo/modif_categoria_centro_costo`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
