...
export default class VisuCompaniaComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-compania';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
        ...
        if(this.compania.strCompany_Cod==this.companyCod){
          await setTimeout(() => {
            ...
            if(this.compania.strCompany_Cod!=''){
              router.push({ path: `/barmenu/XX-CONFI/maestro_datos/compania/modif_compania`, query: { vista: 'visualizar',data:JSON.stringify(this.compania) }  })
            }
          }, 600)
        }
        else{
