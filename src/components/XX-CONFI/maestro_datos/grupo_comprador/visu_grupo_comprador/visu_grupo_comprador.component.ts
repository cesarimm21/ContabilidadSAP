...
@Component({
  name: 'visualizar-grupocomprador',
  components:{
...
export default class VisualGrupoCompradorComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-grupocomprador';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strGrpPurch_Cod==this.strGrpPurch_Cod){
        await setTimeout(() => {
          if(this.documento.strGrpPurch_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/grupo_comprador/modif_grupo_comprador`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
