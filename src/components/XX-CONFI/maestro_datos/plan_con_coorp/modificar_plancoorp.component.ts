...
export default class ModificarPlanCoorporativoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-planconcoorporativo';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
  ...
  async  EliminarItem(){
    if(this.documento.intIdChartAcct_C_ID!=-1&&this.documento.strChartAcct_C_Cod!=""&&this.documento.strChartAcct_C_Desc!=""){
  ...
  inactivarPlan(){
    ...
    planService.inactivarPlanConCoorporativo(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new PlanCuentaCorporativoModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strChartAcct_C_Cod!="" && this.documento.strChartAcct_C_Desc!=""){
      ...
      this.warningMessage('Selecciones plan contable coorporativo')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    planService.activarPlanConCoorporativo(this.documento)
    .then(resp=>{
      loadingInstance.close();
      ...
      if(this.documento.strChartAcct_C_Cod==this.strChartAcct_C_Cod){
        await setTimeout(() => {
          if(this.documento.strChartAcct_C_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/plan_con_coorp/viewandedit_plancoorp`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
