...
export default class VisuCorrelativoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visu-correlativo';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
    ...
    async cargarList(){
        debugger;
        if(this.cod_modulo!=''){
            await correlativoService.GetCorrelativoId(this.cod_modulo)
            .then(res=>{
                debugger;
            ...
            await correlativoService.GetCorrelativoAll()
            .then(res=>{
                debugger;
                console.log('/****************Busqueda***************/')
                console.log(res)
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.intIdInvStock_ID!=-1){
            ...
            if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.strAcc_Local_NO!=''){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/correlativo/modif_correlativo`, query: { vista: 'visualizar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
