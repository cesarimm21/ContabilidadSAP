...
export default class VisuCorrelativoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-correlativo';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.correlativo.strCorrel_Cod==this.strCorrel_Cod){
        await setTimeout(() => {
          ...
          if(this.correlativo.strCorrel_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/correlativo/modif_correlativo`, query: { vista:'visualizar' ,data:JSON.stringify(this.correlativo) }  })
          }
        }, 600)
      }
      else{
