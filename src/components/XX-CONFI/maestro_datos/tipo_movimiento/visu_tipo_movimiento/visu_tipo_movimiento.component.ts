...
@Component({
  name: 'visualizar-tipomovimiento',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisuTipoMovimientoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-tipomovimiento';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strTypeMov_Cod==this.strTypeMov_Cod){
        await setTimeout(() => {
          if(this.documento.strTypeMov_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/tipo_movimiento/modif_tipo_movimiento`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
