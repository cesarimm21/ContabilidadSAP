...
export default class ModificarIdiomaComponent extends Vue {
  ...
  async  EliminarItem(){
    if(this.documento.intLenguaje_ID!=-1&&this.documento.strLenguaje_Cod!=""&&this.documento.strLenguaje_Desc!=""){
  ...
  inactivarPlan(){
    ...
    idiomaService.inactivarIdioma(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new IdiomaModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strLenguaje_Cod!="" && this.documento.strLenguaje_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Idioma')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    idiomaService.activarIdioma(this.documento)
    .then(resp=>{
      loadingInstance.close();
      ...
      if(this.documento.strLenguaje_Cod==this.strLenguaje_Cod){
        await setTimeout(() => {
          if(this.documento.strLenguaje_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/idioma/viewandedit_idioma`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
