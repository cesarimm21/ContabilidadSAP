...
@Component({
  name: 'visualizar-grupo-proceso',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisuGrupoProcesoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-grupo-proceso';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strCCGrpProc_Cod==this.strCCGrpProc_Cod){
        await setTimeout(() => {
          if(this.documento.strCCGrpProc_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/grupo_proceso/modif_grupo_proceso`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
