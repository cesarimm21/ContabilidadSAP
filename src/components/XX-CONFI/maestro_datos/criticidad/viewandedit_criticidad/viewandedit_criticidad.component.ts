...
export default class ViewAndEditCriticidadComponent extends Vue {
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.intIdInvStock_ID!=-1){
            ...
            if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.strAcc_Local_NO!=''){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/criticidad/modif_criticidad`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
  ...
  async btnEliminar(){
    await criticidadService.Eliminar(this.currentRow)
    .then(response=>{
      debugger;
