...
@Component({
  name: 'modificar-doc-transaccion',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class ViewAndEditDocumentoTransaccionComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-doc-transaccion';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
  ...
  async  EliminarItem(){
    if(this.documento.intDoc_Trans_ID!=-1&&this.documento.strDoc_Trans_Cod!=""&&this.documento.strDoc_Trans_Desc!=""){
  ...
  inactivarPlan(){
    ...
    doctransService.inactivarDocumentoTransaccion(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new DocumentoTransacionModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strDoc_Trans_Cod!="" && this.documento.strDoc_Trans_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Documento Transaccion')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    doctransService.activarDocumentoTransaccion(this.documento)
    .then(resp=>{
      loadingInstance.close();
