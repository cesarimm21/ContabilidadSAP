...
@Component({
  name: 'viewandedit-documento-transaccion',
  components:{
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.intIdInvStock_ID!=-1){
            ...
            if(this.selectrow!=undefined && this.selectrow!=null && this.selectrow.strAcc_Local_NO!=''){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/documento_transaccion/modif_documento_transaccion`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
  ...
  async btnEliminar(){
    ...
    .then(response=>{
      ...
      if(response!=undefined){
         this.textosave='Se elimino correctamento.' + response.strDoc_Trans_Cod;
         this.issave=true;
         this.iserror=false;
      }
      else{
