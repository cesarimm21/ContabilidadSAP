...
export default class ViewAndEditTipoOperacionComponent extends Vue {
  ...
  async  EliminarItem(){
    if(this.documento.intIdTypeOper_ID!=-1&&this.documento.strTypeOper_Cod!=""&&this.documento.strTypeOper_Desc!=""){
      this.planDialog=true;
    }
    else{
      this.warningMessage("Selecciona Tipo Operacion")
    }    
  }
  inactivarPlan(){
    ...
    tipooperacionService.inactivarTipooperacion(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new TipoOperacionModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strTypeOper_Cod!="" && this.documento.strTypeOper_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Tipo Operacion')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    tipooperacionService.activarTipooperacion(this.documento)
    .then(resp=>{
      loadingInstance.close();
      ...
      if(this.documento.strTypeOper_Cod==this.strTypeOper_Cod){
        await setTimeout(() => {
          if(this.documento.strTypeOper_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/tipo_operacion/modif_tipo_operacion`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
