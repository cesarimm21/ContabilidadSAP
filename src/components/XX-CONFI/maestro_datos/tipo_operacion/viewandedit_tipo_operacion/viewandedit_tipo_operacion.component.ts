...
@Component({
  name: 'viewandedit-tipooperacion',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions':ButtonsAccionsComponent,
  }
})
export default class ViewAndEditTipoOperacionComponent extends Vue {
  ...
  constructor(){    
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null ){
            debugger;
            if(this.selectrow!=undefined && this.selectrow!=null ){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/tipo_operacion/modif_tipo_operacion`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
  ...
  async btnEliminar(){
    ...
    ActivarDesactivar(){
        debugger;
        this.item=this.selectrow.strTypeOper_Cod;
      ...
      async btnInactivar(){
        ...
        if(this.selectrow.strTypeOper_Cod!=""){
          ...
          await tipooperacionService.activar(this.selectrow)
          .then(respo=>{
            loadingInstance.close();
            this.successMessage('Se Activo Tipo Operacion '+this.selectrow.strTypeOper_Cod)
            this.load();
            this.issave=true;
            this.iserror=false;
            this.textosave='Se Activo Tipo Operacion '+this.selectrow.strTypeOper_Cod;
            this.dialogInactivar=false;
          }).catch(ee=>{
            loadingInstance.close();
            this.issave=false;
            this.iserror=true;
            this.textosave='Error en Activar '+this.selectrow.strTypeOper_Cod;
            this.errorMessage('Error en Activar '+this.selectrow.strTypeOper_Cod)})
            this.dialogInactivar=false;
        }
        else{
