  ...
  constructor(){    
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null ){
            debugger;
            if(this.selectrow!=undefined && this.selectrow!=null ){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/prioridad/modif_prioridad`, query: { vista: 'modificar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
  ...
  async btnInactivar(){
    ...
    if(this.selectrow.strCtlPrec_Cod!=""){
      ...
      .then(respo=>{
        ...
        this.successMessage('Se Activo Tipo Movimiento '+this.selectrow.strPriority_Cod)
        this.load();
        this.issave=true;
        this.iserror=false;
        this.textosave='Se Activo Tipo Movimiento '+this.selectrow.strPriority_Cod;
