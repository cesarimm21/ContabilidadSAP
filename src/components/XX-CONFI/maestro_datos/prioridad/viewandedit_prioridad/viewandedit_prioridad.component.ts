...
export default class ViewAndEditPrioridadComponent extends Vue {
  ...
  async  EliminarItem(){
    if(this.documento.intIdPriority_ID!=-1&&this.documento.strPriority_Cod!=""&&this.documento.strPriority_Desc!=""){
      this.planDialog=true;
    }
    else{
      this.warningMessage("Selecciona un Prioridad")
    }    
  }
  inactivarPlan(){
    ...
    prioridadService.desactivarprioridad(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new PrioridadModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strPriority_Cod!="" && this.documento.strPriority_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Prioridad')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    prioridadService.activar(this.documento)
    .then(resp=>{
      loadingInstance.close();
      ...
      if(this.documento.strPriority_Cod==this.strPriority_Cod){
        await setTimeout(() => {
          if(this.documento.strPriority_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/prioridad/modif_prioridad`, query: { vista:'modificar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
