...
@Component({
  name: 'modificar-tipoadquisicion',
  components:{
...
export default class ModificarAdquisicionoComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='modificar-tipoadquisicion';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
  ...
  async  EliminarItem(){
    if(this.documento.intTypeAdq_PDB_Cod!=-1&&this.documento.strTypeAdq_PDB_Cod!=""&&this.documento.strTypeAdq_PDB_Desc!=""){
      this.planDialog=true;
    }
    else{
      this.warningMessage("Selecciona Tipo Adquisicion")
    }    
  }
  inactivarPlan(){
    ...
    tipoadquisicionService.eliminar(this.documento)
    .then(resp=>{
      ...
      this.$message({
        ...
        this.documento=new TipoAdquisicionModel();
        this.load();
        this.issave = true;
        this.iserror = false;
  ...
  async Activar(){
    if(this.documento.strTypeAdq_PDB_Cod!="" && this.documento.strTypeAdq_PDB_Desc!=""){
      this.planActivarDialog=true;
    }
    else{
      this.warningMessage('Selecciones Tipo Adquisicion')
    }
  }
  activarPlan(){
    this.nameuser=localStorage.getItem('User_Usuario');
    ...
    tipoadquisicionService.activar(this.documento)
    .then(resp=>{
      loadingInstance.close();
