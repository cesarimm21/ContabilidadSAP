...
export default class ViewAndEditMetodoDepComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='viewandedit-metododep';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
    load(){
        ...
        if(vista=='modificar'){
            ...
            this.textTitle='Modificar Metodo Depreciacion';
        }
        if(vista=='visualizar'){
            this.enabledtf=true;
            this.textTitle='Visualizar Metodo Depreciacion';
        }
    ...
    guardarTipoDocIdent(){
      ...
      if(vista=='modificar'){
        ...
        if(this.documento.strDeprMeth_Cod!=''&&this.documento.strDeprMeth_Desc!=''){
          metodoService.UpdateMetodoDep(this.documento)
          .then(resp=>{
