...
export default class VisuCategoriaCuentaComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visu-categoriacuenta';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
    ...
    async validarView(){
        ...
        if(this.selectrow!=undefined && this.selectrow!=null ){
            debugger;
            if(this.selectrow!=undefined && this.selectrow!=null ){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/categoria_cuenta/modif_categoria_cuenta`, query: { vista: 'visualizar',data:JSON.stringify(this.selectrow) }  })
            }
        }
        else{
