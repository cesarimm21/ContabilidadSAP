...
@Component({
  name: 'visualizar-categoriacuenta',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions':ButtonsAccionsComponent,
  }
})
export default class VisuCategoriaCuentaComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-categoriacuenta';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
    ...
    async validarView(){
        if(this.categoriacuenta.strAcctCateg_Cod!='' && this.categoriacuenta.intIdAcctCateg_ID!=-1 ){
            if(this.categoriacuenta.strAcctCateg_Cod!='' && this.categoriacuenta.intIdAcctCateg_ID!=-1){
                router.push({ path: `/barmenu/XX-CONFI/maestro_datos/categoria_cuenta/modif_categoria_cuenta`, query: { vista: 'visualizar',data:JSON.stringify(this.categoriacuenta) }  })
            }
        }
        else{
