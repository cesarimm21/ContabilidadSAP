...
@Component({
  name: 'visualizar-tiporenta',
  components:{
  'quickaccessmenu':QuickAccessMenuComponent,
  'buttons-accions': ButtonsAccionsComponent,
  }
})
export default class VisuTipoRentaComponent extends Vue {
  ...
  constructor(){    
        super();
        Global.nameComponent='visualizar-tiporenta';
        setTimeout(() => {
            this.load();
          }, 200)
    }  
      ...
      if(this.documento.strReveType_Cod==this.strReveType_Cod){
        await setTimeout(() => {
          if(this.documento.strReveType_Cod!=''){
            router.push({ path: `/barmenu/XX-CONFI/maestro_datos/tipo_renta/modif_tipo_renta`, query: { vista:'visualizar' ,data:JSON.stringify(this.documento) }  })
          }
        }, 600)
      }
      else{
